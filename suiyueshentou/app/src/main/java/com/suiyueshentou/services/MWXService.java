...
import com.suiyueshentou.utils.HongbaoSignature;
...
public class MWXService extends AccessibilityService {
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        /* 检测通知消息 */
        if (!mMutex) {
            if (watchedFlags.get("pref_watch_notification") && watchNotifications(event))
                return;
            if (watchedFlags.get("pref_watch_list") && watchList(event))
                return;
        }

        if (!watchedFlags.get("pref_watch_chat"))
            return;
    ...
    private void checkNodeInfo() {
        ...
        if (mLuckyMoneyPicked) {
            List<AccessibilityNodeInfo> nodes3 = this.findAccessibilityNodeInfosByTexts(this.rootNodeInfo,
                    new String[] { WECHAT_BETTER_LUCK_CH, WECHAT_DETAILS_CH, WECHAT_BETTER_LUCK_EN, WECHAT_DETAILS_EN,
                            WECHAT_EXPIRES_CH });
            if (!nodes3.isEmpty()) {
                mNeedBack = true;
